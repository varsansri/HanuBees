...
> **🤖 AI operators (opencode etc.): read [`AGENTS.md`](./AGENTS.md) FIRST.** It is the
> operating manual. The hardcoded source of truth is [`scripts/config.js`](./scripts/config.js).
> Control it with keywords: `node scripts/army.js status` · `node scripts/army.js next` (one batch of 10) · `plan` · `subjects`.
>
> This is the **opencode-driven twin** of `../hanubees-marketing-team`. It runs **alongside** the
> original on the **same 8 accounts (for now)**; the shared `subject_log` table + `scripts/claim.js`
> stop both from posting the same subject. Format: Instagram/TikTok/YouTube = **video+music**,
> Threads = **image carousel**. Volume: **≥13/account/day → 130+/day**, anti-burst random gaps.
> Self-contained: scripts read THIS folder's own `.env.local` via `scripts/_env.js`.
